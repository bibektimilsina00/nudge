 ...
 * The landing page.
 *
 * Written around one claim, because a page that makes four claims makes none.
 * The claim is that Nudge can see the screen and act on it -- which is both the
 * product and the thing people have to be persuaded to allow, so it is said
 * plainly at the top rather than eased into.
 */
  ...
  return (
    <header className="flex items-center justify-between py-6">
      <div className="flex items-center gap-2.5">
        ...
        <img src="/icon.png" alt="" className="size-8 rounded-[7px]" />
        <span className="text-[15px] font-semibold tracking-tight">Nudge</span>
      </div>
      <a
...
function Questions() {
  const qa = [
    ...
    {
      q: "Can I use it while it is working?",
  ...
  return (
    <section className="border-t py-16">
      <h2 className="text-xl font-semibold tracking-tight">Before you download it</h2>
...
function Footer() {
  return (
    <footer className="mt-auto flex flex-col items-center gap-2 border-t py-10 text-xs text-muted-foreground">
      <p>Nudge is an early build. Things will break, and it will tell you when they do.</p>
      <p>© {new Date().getFullYear()} Nuddg Inc</p>
    </footer>
  );
}
