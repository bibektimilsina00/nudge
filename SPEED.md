...
| **Capture** | ~~`CGWindowListCreateImage`~~ **`SCScreenshotManager`, done** | `SCStream` always running -- a frame is already in hand | done | **1769ms → 61ms per capture.** Composited, scaled and encoded by ScreenCaptureKit and ImageIO; the pixels never pass through Rust | **Ahead of the plan.** `SCStream` was the plan and is now the wrong build: it would remove the 56ms composite and leave the encode, and it pays for that by capturing the screen continuously. Asking for a frame is cheaper than keeping one warm |
