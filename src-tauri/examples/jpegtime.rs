...
use objc2_image_io::{CGImageDestinationCreateWithData, CGImageDestinationAddImage, CGImageDestinationFinalize};
...
fn main() {
    ...
    let img = nudge_lib::core::screen::fast::grab(id, 1280, std::process::id() as i32).expect("frame");
    let composited = began.elapsed().as_secs_f32() * 1000.0;
    let t = Instant::now();
    let mut bytes = Vec::new();
    img.write_with_encoder(image::codecs::jpeg::JpegEncoder::new_with_quality(
        &mut std::io::Cursor::new(&mut bytes), 78)).expect("encode");
    println!("  ours    composite+convert {composited:>6.0}ms   encode {:>6.0}ms   {:>4}KB",
        t.elapsed().as_secs_f32() * 1000.0, bytes.len() / 1024);

    // Theirs: hand the CGImage straight to ImageIO.
    let began = Instant::now();
    let cg = nudge_lib::core::screen::fast::frame(id, 1280, std::process::id() as i32).expect("frame");
    let composited = began.elapsed().as_secs_f32() * 1000.0;
    let t = Instant::now();
    let data = CFMutableData::new(None, 0).expect("data");
    let kind = CFString::from_static_str("public.jpeg");
    let dest = unsafe { CGImageDestinationCreateWithData(&data, &kind, 1, None) }.expect("dest");
    ...
    let props = CFDictionary::from_slices(&[key], &[quality.as_ref() as &objc2_core_foundation::CFType]);
    let props = props.as_opaque();
    unsafe { CGImageDestinationAddImage(&dest, &cg, Some(&props)) };
    let ok = unsafe { CGImageDestinationFinalize(&dest) };
    println!("  theirs  composite         {composited:>6.0}ms   encode {:>6.0}ms   {:>4}KB  ok={ok}",
        t.elapsed().as_secs_f32() * 1000.0, data.len() as usize / 1024);
}
